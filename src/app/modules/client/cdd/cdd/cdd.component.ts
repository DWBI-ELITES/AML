...
export class CddComponent implements OnInit {
  ...
  public searchResults: { cust_id: string, account: string, name: string }[] = [];

  currentPage: number = 1; // Current page
  itemsPerPage: number = 3; // Number of records per page
  ...
  showNoFilterCriteriaWarning: boolean = false; // Added this line

  showNoSearchCriteriaWarning: boolean = false; // Added this line

  showNoFilterRecordWarning: boolean = false;
...
getTotalPages() {
  if (this.filteredResults.length === 0) {return Math.ceil(this.searchResults.length / this.itemsPerPage);}
  else {return Math.ceil(this.filteredResults.length / this.itemsPerPage);}
...
// Function to calculate the start and end index for records to display
getRecordRange() {
  ...
  clearInput(fieldName: string) {
    ...
    if (fieldName === 'accountSearchValue') {
      ...
      this.showNoFilterCriteriaWarning = false;
    this.showNoFilterRecordWarning = false;
...
filteredResults: any[] = []; // Array to hold filtered results
  ...
  filterTable() {
    ...
    if (this.buttonClicked = true && this.filteredResults.length < 1)
      { this.showNoFilterRecordWarning = true}

    this.filteredResults = this.searchResults.filter((result) => {
      const resultNameWords = result.name.trim().toLowerCase().split(' '); // Split result.name into words
      const newnamesearchValue = this.nameSearchValue.trim().toLowerCase();
      // Check if any of the words in resultNameWords are equal to searchValue
      const found = resultNameWords.some(word => word === newnamesearchValue);
      

     
      if (this.custIDSearchValue === '' && this.accountSearchValue === '' && this.nameSearchValue === '')
      { this.showNoFilterCriteriaWarning = true;}
      if( result.cust_id === this.custIDSearchValue)
      {return result.cust_id.includes(this.custIDSearchValue)}
      if( result.account === this.accountSearchValue)
      {return result.account.includes(this.accountSearchValue)}
      if( result.name.trim().toLowerCase() === this.nameSearchValue.trim().toLowerCase() )
      {return result.name.includes(this.nameSearchValue)}
      if (found) {
       return resultNameWords
      }
  ...
  //   // Search terms
  //   const custIDSearchTerms = this.custIDSearchValue.trim().toLowerCase().split(' ');
  //   const accountSearchTerms = this.accountSearchValue.trim().toLowerCase().split(' ');
  //   const nameSearchTerms = this.nameSearchValue.trim().toLowerCase().split(' ');
  ...
  //     if (result.cust_id === this.custIDSearchValue) {
  //       isMatch = true;
  //     } else if (result.account === this.accountSearchValue) {
  //       isMatch = true;
  //     } else {
  //       const nameWords = result.name.trim().toLowerCase().split(' ');
  
  //       // Check if this.nameSearchValue matches any word in result.name
  //       if (nameWords.some((word) =>
  //         word.includes(this.nameSearchValue.trim().toLowerCase())
  //       )) {
  //         isMatch = true;
  //       }
  //     }
  ...
  ngOnInit() {
    ...
    // throw new Error('Method not implemented.');
        ...
        .subscribe(data => {
            ...
            this.http.get<{ cust_id: string, name: string, account: string }[]>('https://localhost:44356/api/AccountSearch/Search')
            .subscribe(data => {
